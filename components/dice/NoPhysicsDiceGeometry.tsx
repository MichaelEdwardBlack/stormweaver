...
function FaceText({ geometry, labels }: { geometry: THREE.BufferGeometry; labels: string[] }) {
  ...
  (geometry as any).userData.faceInfos = unique.map((f) => ({
...
export const D6 = (props: any) => {
  ...
  return (
    <Box args={[radius, radius, radius]} {...props} castShadow receiveShadow>
      {Array.from(Array(6)).map((_, i) => (
        <meshPhongMaterial key={i} attach={`material-${i}`} color={diceColor} />
      ))}
      {/* Numbers on each face */}
      {/* Front (z+) */}
      <Text position={[0, 0, radius / 2 + 0.01]} fontSize={0.8} color={diceText}>
        1
      </Text>
